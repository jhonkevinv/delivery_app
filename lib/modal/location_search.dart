...
import 'package:flutter_maps_example/modal/picker_location.dart';
...
class LocationSearchWidget extends StatefulWidget {
  ...
  const LocationSearchWidget({
    ...
    this.searchBarHintText = 'Buscar ubicación',
    this.currentPositionButtonText = 'Usar ubicación actual',
...
class _LocationSearchWidgetState extends State<LocationSearchWidget> {
          ...
          return ListTile(
            ...
            onTap: () async {
              if (_options.isNotEmpty) {
                final historyJson = await _historyManager.getHistory();
                List<dynamic> historyList2 = historyJson
                    .map(
                      (loc) => jsonDecode(loc),
                    )
                    .toList();
                if (!historyList2.contains(items[index].address)) {
                  _addToHistory(items[index]);
                }
              }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildSelectPointPositionButton(_options),
          Expanded(
      ...
      child: TextButton(
        onPressed: (() async {
          ...
          List<dynamic> historyList2 = historyJson
              ...
              .toList();
          if (!historyList2.contains(currentPos.address)) {
  ...
  Widget _buildSelectPointPositionButton(options) {
    return Container(
      padding: const EdgeInsets.symmetric(vertical: 5),
      margin: const EdgeInsets.symmetric(vertical: 5),
      decoration: BoxDecoration(
      ...
      child: TextButton(
        onPressed: (() async {
          final items = _options.isEmpty ? _history : _options;
          var p = await showSimplePickerLocation2(
            context: context,
            isDismissible: true,
            title: "Seleccionar ubicación",
            textConfirmPicker: "Elegir",
            textCancelPicker: "Cancelar",
            zoomOption: ZoomOption(
              initZoom: 18,
            ),
            initPosition: _options.isEmpty
                ? GeoPoint(
                    latitude: -12.072595,
                    longitude: -77.087111,
                  )
          ...
          if (p != null) {
            ...
            String url =
                "https://nominatim.openstreetmap.org/reverse?format=json&lat=${p.latitude}&lon=${p.longitude}&zoom=18&addressdetails=1&accept-language=${widget.language}'}";
            ...
            try {
              ...
              LocationData z = LocationData(
                  latitude: p.latitude,
                  longitude: p.longitude,
                  address: posData.address,
                  addressData: posData.addressData);
              ...
              List<dynamic> historyList2 = historyJson
                  ...
                  .toList();
              if (!historyList2.contains(z.address)) {
                _addToHistory(z);
              }
              widget.onPicked!(z);
            } on Exception catch (e) {
...
class LocationSearch {
  static Future<LocationData?> show({
    ...
    String? searchBarHintText = 'Buscar ubicación',
    String? currentPositionButtonText = 'Usar ubicación actual',
